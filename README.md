# HTTP Error Pages
This repo contains modern-looking dark-mode-ready HTTP error pages written in HTML and styled using a single CSS file.

## Get started
Simply clone this repo, put the files into the directory of your liking, replace `%home_url%` in all files with your own home URL and get going.  
Optionally, you can change the `style.css` link in every `<head>` section to match your folder structure.  
After that, set up your server to serve the pages upon encountering an error and **you're good to go**!  
  
## License
This project is licensed under the GNU General Public License, more on that in the LICENSE file.

## Support
If you wanna thank me, just leave me an e-mail with a quick thanks. You can find it [on my website](https://oau.cz).

## Screenshots
<img width="1470" height="729" alt="dark-451" src="https://github.com/user-attachments/assets/b7f14d23-555d-43c8-90af-ba4c9a483eed" />
<img width="1470" height="732" alt="light-504" src="https://github.com/user-attachments/assets/ae028b8a-e11c-48f0-9d7c-bc1af3150bc2" />

### That's all! Happy coding :)
